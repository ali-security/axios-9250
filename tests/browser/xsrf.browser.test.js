...
describe('xsrf (vitest browser)', () => {
  ...
  describe('GHSA-xx6v-rp6x-q39c non-boolean withXSRFToken', function () {
    ...
    leakCases.forEach(([label, value]) => {
      it(`should not send xsrf header cross-origin when withXSRFToken = ${label}`, async function () {
        setXsrfCookie('12345');
    ...
    it('should not send xsrf header cross-origin when Object.prototype.withXSRFToken is polluted', async function () {
    ...
    it('should still send xsrf header cross-origin when withXSRFToken === true (strict)', async function () {
      const token = '12345';
      setXsrfCookie(token);
    ...
    it('should still send xsrf header same-origin when withXSRFToken is undefined', async function () {
      const token = '12345';
      setXsrfCookie(token);
      const request = await sendRequest('/foo');
      expect(request.requestHeaders[axios.defaults.xsrfHeaderName]).toBe(token);
